...
const aVGSessionLength = `-- name: AVGSessionLength :one
SELECT AVG(session_length_seconds) AS avg_session_length
FROM sessions
WHERE session_length_seconds IS NOT NULL
`

func (q *Queries) AVGSessionLength(ctx context.Context) (float64, error) {
	row := q.db.QueryRow(ctx, aVGSessionLength)
	var avg_session_length float64
	err := row.Scan(&avg_session_length)
	return avg_session_length, err
}
